...
test.beforeEach('Create test geometry', t => {
  t.context.polyline = Polyline.fromCoords([[0,0],[3,3],[6,0]], true);
  t.context.hole = Polyline.fromCoords([[2,1],[4,1],[3,2]], true);
...
test('Constructor: Using an open polyline throws error', t => {
  const polyline = Polyline.fromCoords([[0,0],[3,3],[6,0]], false);
  t.throws(() => {
...
test('Constructor: Changes orientation of boundary to be clockwise', t => {
  ...
  const polyline = Polyline.fromCoords([[0,0],[6,0],[3,3]], true);
...
test('Constructor: Using an open polyline for a hole throws error', t => {
  const polyline = Polyline.fromCoords([[0,0],[3,3],[6,0]], true);
  const hole = Polyline.fromCoords([[2,1],[3,2],[4,1]], false);
  t.throws(() => {
    // tslint:disable-next-line:no-unused-expression
...
test('Constructor: Changes curve orientation of hole to be anti-clockwise', t => {
  const polyline = Polyline.fromCoords([[0,0],[3,3],[6,0]], true);
  const hole = Polyline.fromCoords([[2,1],[3,2],[4,1]], true);
...
test('fromCoords: Can create polygon from coordinates', t => {
  const polygon = Polygon.fromCoords([[[0,0],[3,3],[6,0],[0,0]], [[2,1],[3,2],[4,1],[2,1]]]);
...
test('contains: point outside triangle', t => {
  t.is(t.context.triangleHole.contains(new Point(3, 10)), PointContainment.outside);
});
test('contains: point on outside triangle', t => {
  t.is(t.context.triangleHole.contains(new Point(3, 3)), PointContainment.coincident);
});
test('contains: point inside triangle', t => {
  t.is(t.context.triangleHole.contains(new Point(3, 2.5)), PointContainment.inside);
});
test('contains: edge of hole', t => {
  t.is(t.context.triangleHole.contains(new Point(3, 1)), PointContainment.coincident);
});
test('contains: inside hole', t => {
  t.is(t.context.triangleHole.contains(new Point(3, 1.1)), PointContainment.outside);
});
...
test('Union: two adjacent rectangles join to become one', t => {
  const rectA = new Polygon(Rectangle.fromCenter(Plane.worldXY(), 10, 20).toPolyline());
...
test('Union: a big rectangle over the trinagle returns the big rectangle', t => {
  const rectA = new Polygon(Rectangle.fromCenter(Plane.worldXY(), 20, 20).toPolyline());
...
test('intersection: rectangle over half the triangle should return half triangle', t => {
  const rectA = new Polygon(Rectangle.fromCenter(Plane.worldXY(), 6, 20).toPolyline());
...
test('difference: can cut a hole out of another rectangle', t => {
  const rectA = new Polygon(Rectangle.fromCenter(Plane.worldXY(), 10, 10).toPolyline());
  const rectB = new Polygon(Rectangle.fromCenter(Plane.worldXY(), 1, 1).toPolyline());
  const result = rectA.difference(rectB);
