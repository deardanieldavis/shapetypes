...
test('closestPoint: works when point is inside the rectangle', t => {
  ...
  t.is(t.context.basic.closestPoint(point).x, 5);
  t.is(t.context.basic.closestPoint(point).y, 6);
});
test("closestPoint: works when point is inside the rectangle but interior isn't included", t => {
  const point = new Point(5, 6);
  t.is(t.context.basic.closestPoint(point, false).x, 5);
  t.is(t.context.basic.closestPoint(point, false).y, 5);
});
