...
test('Union: two rectangles that dont overlap, wont union', t => {
  ...
  const rectB = Rectangle.fromCenter(center, 10, 20).toPolyline();
  const result = t.context.rect.union(rectB);

  t.is(result.length, 2);
  if (!(result[0] instanceof Polyline) || !(result[1] instanceof Polyline)) {
    throw new Error('Should be polyline');
  }
  t.true(result[0].equals(t.context.rect));
  t.true(result[1].equals(rectB));
});
...
test('Union: one rectangle inside another returns the larger one', t => {
  ...
  const rectB = Rectangle.fromCenter(center, 5, 10).toPolyline();
  const result = t.context.rect.union(rectB);
...
test('union: throws error if applied to an open curve', t => {
  t.throws(() => {
    t.context.rect.union(t.context.elbow);
  });
});

test('intersection: two rectangles that dont overlap return nothing', t => {
  const center = new Plane(new Point(100, 0), Vector.worldX());
  const rectB = Rectangle.fromCenter(center, 10, 20).toPolyline();
  const result = t.context.rect.intersection(rectB);

  t.is(result.length, 0);
});
test('intersection: one rectangle inside another returns the smaller one', t => {
  const center = new Plane(new Point(0, 0), Vector.worldX());
  const rectB = Rectangle.fromCenter(center, 5, 10).toPolyline();
  const result = t.context.rect.intersection(rectB);
...
test('intersection: throws error if applied to an open curve', t => {
  t.throws(() => {
    t.context.rect.intersection(t.context.elbow);
  });
});

test('difference: two rectangles that dont overlap return same shape', t => {
  const center = new Plane(new Point(100, 0), Vector.worldX());
  const rectB = Rectangle.fromCenter(center, 10, 20).toPolyline();
  const result = t.context.rect.difference(rectB);
...
test('difference: throws error if applied to an open curve', t => {
  t.throws(() => {
    t.context.rect.difference(t.context.elbow);
  });
});
